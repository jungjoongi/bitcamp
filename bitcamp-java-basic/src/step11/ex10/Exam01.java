...
public class Exam01 {
    public static void main(String[] args) {
        ...
        // 메서드를 호춯할 때 넘겨주는 값(아규먼트)의 타입으로 과 개수로 호출할 메서드를 결정한다.

        
        // 다음은 m()메서드 중에서 아규먼트 를 받지 않는 메서드를 호출한다.
        A.m();
        ...
        // 메서드를 찾을 때 아규먼트의 타입과 일이하는 메서드를 차기 때문에
        // 메서드의 파라미터 으름은 아무 상관이 없다.
    }
}

// 오버로딩(overloading)
// -> 파라미터 타입이나 개수, 순서는 다르지만 이름이 같은 메서드를 여러개 만들 수 있는 문법.
// 왜 이런 문법이 등장했는가?
